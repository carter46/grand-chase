...
Route::middleware(['isadmin', '2fa'])->prefix('admin')->group(function () {
	...
	// user-wallet route removed — ManageUsersController::userwallet was missing (would 500)
	...
	Route::get('dashboard/viewagent/{agent}', [HomeController::class, 'viewagent'])->name('viewagent');
	Route::get('dashboard/delagent/{id}', [LogicController::class, 'delagent'])->name('delagent');
	// Legacy SettingsController routes removed (class missing). Use AppSettingsController / PaymentController / ManageAssetController instead.

	Route::post('dashboard/clearcache', [ClearCacheController::class, 'clearcache'])->name('clearcache');
	...
	Route::get('dashboard/confirmsub/{id}',  [SubscriptionController::class, 'confirmsub'])->name('confirmsub');
	...
	// approveplan route removed — ManageUsersController::approvePlan was missing (would 500)
