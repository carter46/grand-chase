...
class SettingsServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        try {
            ...
            Log::error('Failed to load application settings from database.', [
        ...
        if (!$settings || !$paystack || !$settings2) {
            Log::error('Missing required settings entries for application boot.', [
                'has_settings' => (bool) $settings,
                'has_paystack' => (bool) $paystack,
                'has_settings_cont' => (bool) $settings2,
            ]);
