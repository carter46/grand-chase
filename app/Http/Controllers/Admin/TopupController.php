...
class TopupController extends Controller
{
   use PingServer;
    ...
    public function topup(Request $request)
    {
        ...
        if (!$settings) {
            return redirect()->back()->with('message', 'System settings are unavailable. Please try again later.');
        }
        ...
        $user_deposit = $userdpo ? $userdpo->amount : 0;
        ...
        try {
            ...
            $mailError = 'Unable to send credit notification email to the user. Their balance was still updated successfully.';
            Log::error('Failed to send top-up credit notification email.', [
                'user_id' => $user->id,
     ...
     \nAvailable Balance:$bala$settings->s_currency";
        ...
        try {
          ...
          if ($request['type']=="Bonus") {
           ...
           try {
               ...
               $mailError = 'Unable to send debit notification email to the user. Their balance was still updated successfully.';
               Log::error('Failed to send top-up debit notification email.', [
                   'user_id' => $user->id,
                   'user_email' => $user->email,
                   'amount' => $request->amount,
                   'error' => $exception->getMessage(),
        ...
        $response = redirect()->back()->with('success', 'Action Successful!');

        if ($mailError) {
            $response->with('message', $mailError);
        }
